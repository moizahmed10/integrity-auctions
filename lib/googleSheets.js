...
export async function getPageData(sheetName) {
  ...
  // Read sections starting from row 2 (header in row 1) in columns B-H
  const sectionsRes = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${sheetName}!B2:H1000`,
  });
  ...
  const sections = rows
    ...
    .map((row) => ({
      ...
      topSpacing: row[4] !== undefined && row[4] !== "" ? Number(row[4]) : 0,
      bottomSpacing: row[5] !== undefined && row[5] !== "" ? Number(row[5]) : 0,
      headingGap: row[6] !== undefined && row[6] !== "" ? Number(row[6]) : 10,
    }));
...
export async function savePageData(sheetName, sections, displayPage, action) {
  ...
  const normalizedSections = sections.map((section) => ({
    ...
    topSpacing: section.topSpacing ?? 0,
    bottomSpacing: section.bottomSpacing ?? 0,
    headingGap: section.headingGap ?? 10,
  }));
...
export async function getConfigData() {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${SHEET_NAMES.CONFIG}!A2:E2`,
  });
